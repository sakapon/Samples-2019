...
namespace UnitTest
{
	public static class BinarySearch
	{
		// 条件を満たす先頭の番号を探索します。
		// f に r は渡されません。
		public static int Search(Func<int, bool> f, int l, int r)
		{
			int m;
		...
		// 条件を満たす最後の番号を探索します。
		// f に l は渡されません。
		public static int SearchLast(Func<int, bool> f, int l, int r)
		{
			int m;
		...
		public static int IndexForInsert(IList<int> a, int v) => Search(i => a[i] > v, 0, a.Count);

		// Array.BinarySearch メソッドと異なる点: 一致する値が複数存在する場合は先頭の番号。
		public static int Index(IList<int> a, int v)
		{
		...
		public static int IndexLast(IList<int> a, int v)
		{
